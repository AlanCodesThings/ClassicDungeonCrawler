...
// E — power shot (hold to charge, release to fire; blocks all input while charging)
...
if (charging) {
	...
	if (!keyboard_check(ord("E"))) {
		...
		for (var _i = 0; _i < array_length(cells); _i++) {
			...
			// At max charge: also hit perpendicular tiles
			if (lvl >= 6) {
...
if (mouse_check_button_pressed(mb_left)) {
	...
	var cells = grid_line_cells(grid_x, grid_y, snap.dx, snap.dy, range);
	for (var _i = 0; _i < array_length(cells); _i++) {
		var dist_bonus = 1.0 + _i * 0.1; // damage scales with distance
		var hit_dmg    = round(damage * min(dist_bonus, 2.0));
		// Pin effect from previous dodge
		...
		if (_hit_eid != noone) {
			...
			if (pin_shot_ready && variable_instance_exists(_hit_eid, "turns_until_attack")) {
				_hit_eid.turns_until_attack = 3; // pinned for 3 turns
			}
			pin_shot_ready = false;
			if (!ult_active) break; // piercing only during ult
		}
	}
	process_turn();
	move_anim_timer = MOVE_ANIM_DELAY;
